...
if [ ! -n "$CHINAIP" ]; then
  ver=$(wget --no-check-certificate -qO- https://api.github.com/repos/wangyu-/UDP2raw/releases/latest | grep 'tag_name' | cut -d\" -f4)
  [ -z ${ver} ] && echo "Error: Get UDP2raw latest version failed" && exit 1
  
  UDP2RAW_URL=https://github.com/wangyu-/udp2raw/releases/download/${ver}/udp2raw_binaries.tar.gz
  ...
  UDPSPEEDER_URL="https://github.com/wangyu-/UDPspeeder/releases/download/${ver}/speederv2_binaries.tar.gz"
  else
